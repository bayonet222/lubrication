...
pscale = 6*mu*U*L/H^2; 
pcav = 30e3/pscale; % cav pressure 30[kPa]
pchamber = 6E6/pscale; % combustion pressure 6[MPa]

atm = 101.33e3/pscale; % 1 atm = 101.33[kPa]
...
pn = xx*0+atm;  pn(1) = atm; pn(end)= atm;
thetan = (fluidleft+htex)./hhn; thetan(thetan>1.0) = 1.0;
thetan(1) = 1.0;
%%
...
for it = 2:NT
    ...
    while ( (res > tol) && (resR > tolR) )
      ...
      %  if (t>dt), pk = sol_e(xx,beta,t*S,Fb(S*t,beta),h1,h2); end
        ...
        IA = find((pk(2:end-1) > pcav) | (thetak(2:end-1) >=1)) + 1;
        ...
        thetak( IA(pk(IA) >= pcav) ) = 1.0; pk(IA(pk(IA) < pcav)) = pcav; % cavitation check
        
        %% THETA
        IC = find((pk(2:end-1) <= pcav) | (thetak(2:end-1)<1)) + 1;
        ...
        pk(IC(thetak(IC)<1)) = pcav; thetak(IC(thetak(IC)>=1)) = 1; % cavitation check
