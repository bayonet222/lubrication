...
for it = 2:NT
    ...
    while ( (res > tolR) || isnan(res))
      ...
      %  if (t>dt), pk = sol_e(xx,beta,t*S,Fb(S*t,beta),h1,h2); end
        ...
        PK = -kt*( ck(IA)-cm(IA) ) - S*dx*( ck(IA)-ck(IA-1) ) ... %IMPLICIT
        ...
        thetak(end) = kt*cm(end) + S*dx*ck(end-1) + sn(end)*(0-pk(end))... %IMPLICIT
             -sn(end-1)*(0-pk(end-1));
        thetak(end) = thetak(end) ./ ((dx*S+kt)*hhn(end)); %IMPLICIT
        ...
        if mod(iter,100) == 0
            ...
            res = norm(LHS-RHS,'inf') / ( norm(LHS,'inf')+norm(RHS,'inf') );
        end
