...
%% TEXTURES AND PAD
padpos = 1.0; per = 0.2*20; l1 = 0.2; dep = 1.0;
...
for it = 2:NT
    ...
    b = [pk(1) S*dx*(hhm(I)-hhm(I-1)) + kt*(hh(I)-hhm(I)) pk(end)]';%inestable
    %b = [pk(1) S*dx*(hhm(I)-hhm(I-1)) - 2*S*dx*(htex(I)-htex(I-1)+htexm(I)-htexm(I-1))/2 pk(end)]';
  ...
  %  end
    ...
    while (change > tol) %% FIXED POINT RESOLUTION METHOD
