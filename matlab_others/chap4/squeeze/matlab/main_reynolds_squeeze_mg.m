...
T0 = 0.0; Tfinal = 0.5  ; NT = floor((Tfinal-T0)/dt);
...
for it = 2:NT
    ...
    while (change > tol)
        ...
        pk{1} = gsC(b{1}, dx(1), mgItD(1), pk{1}, 1 );
        
        for img = 2:NMg
            b{img} = interp_fina( residueC(b{img-1}, pk{img-1}, dx(img-1)) );
            pk{img} = gsC(b{img}, dx(img), mgItD(img), pk{img}, 0);
        end
        
        for img = NMg:-1:2
            pk{img-1} = pk{img-1} + interp_gruesa( pk{img} );
        end
