...
\label{chap:appendix_maths} % For referencing this appendix elsewhere, use \ref{AppendixA}
...
%Remember a normed space $(X,\|\cdot\|)$ is complete if for every converging sequence $(x_n)\rightarrow x$, we have $x\in X$.
\begin{theorem}[Fischer-Riez]Let $p\in [1,\,\infty]$, $\Omega\subset \mathbb{R}^n,\,\mu(\Omega)>0$. Then $L^p(\Omega)$ is a Banach space with the corresponding norm defined above.
\end{theorem}
...
\begin{theorem}$\wmp{m}{p}$ is separable for $1\leq p < \infty$; also, it is uniformly convex, and thus reflexive if $1<p<\infty$. In particular, $W^{m,2}(\Omega)$ is a separable Hilbert space with inner product
